...
class Cart extends Component
{
    ...
    {
        $products = ProductModels::where('name','like', '%'.$this->search.'%')->orderBy('created_at', 'DESC')->paginate(6);
    ...
    public function addItem($id){
        ...
        $product = ProductModels::find($idProduct);

        if($cekItemId->isNotEmpty()){
            if($product->qty == $cekItemId[$rowId]->quantity || $product->qty == 0){
        ...
        if($kembalian >= 0){
            ...
            try {
                ...
                foreach ($filterCart as $cart) {
                    $product = ProductModels::find($cart['id']);
